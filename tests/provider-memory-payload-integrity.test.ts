...
assert(/^eq64:[0-9a-f]{16}$/.test(parsedA.publicPayloadFingerprint ?? ""), "Modern actual Prompt must carry a 64-bit equality fingerprint for exact CONSULTATION_EVENTS_JSON serialization.");
...
// Most important denominator regression: the transport turn exists, but the
// Prompt-memory observer did not successfully attach its receipt. This seat must
// remain visible and force payload_unverified rather than making the remaining
// two seats appear to be a complete 2/2 verified round.
...
console.log("✓ exact serialized public payload equality is audited separately from event-id deck equality");
console.log("✓ missing Prompt receipts remain in the denominator; repair must preserve payload plus selection provenance");

function prompt(actorId: string, payload: unknown[]): string {
  return promptWithRawPayload(actorId, JSON.stringify(payload));
}
