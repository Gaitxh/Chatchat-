...
import { deriveOpenMeetingIssues, references } from "../src/consultation/open-issues.js";
...
assert(ids.has("q-open"), "Unanswered question must stay open.");
assert(!ids.has("q-answered"), "Question explicitly targeted by later evidence must close.");
assert(ids.has("challenge-open"), "Unanswered challenge must stay open.");
assert(!ids.has("challenge-resolved"), "Challenge causing a later revision must close.");
assert(ids.has("e-waiting"), "Evidence with no explicit response must remain awaiting response.");
assert(!ids.has("e-used"), "Evidence explicitly supported later must close its awaiting-response issue.");
assert(ids.has("uncertain-open"), "Unresolved explicit uncertainty must remain visible.");
assert(!ids.has("uncertain-resolved"), "Later confident non-Uncertain final position by same actor must close uncertainty.");
...
assert(references(events.find((item) => item.id === "revision-b")!).includes("challenge-resolved"), "Revision references must expose causedBy ids.");
assert(references(events.find((item) => item.id === "e-answer")!).includes("q-answered"), "Evidence references must expose target ids.");
