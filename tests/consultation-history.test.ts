...
const events: CouncilEvent[] = [
  ...
  { id: "e2c", sessionId: report.sessionId, round: 2, actorId: "c", kind: "challenge", targetEventId: "e2", content: "The source may not prove that stronger product claim.", createdAt: "2026-08-14T00:00:01.500Z" },
...
assert(archive.events.length === 5, "archive keeps the full event stream");
assert(summary.eventCount === 5, "summary counts events");
assert(summary.mode === "verify", "history summary keeps the public Proposal Mode identity");
assert(summary.challengeCount === 1, "history summary counts structured challenges");
assert(summary.revisionCount === 1, "summary counts revisions");
assert(summary.evidenceCount === 1, "summary counts evidence");
assert(summary.concessionCount === 0, "history summary exposes concession count even when zero");
...
console.log("✓ ChatChat Consultation History meeting-identity tests passed");
