...
#[derive(Debug, Deserialize)]
struct BrowserResponseSnapshot {
    ok: bool,
    count: usize,
    text: String,
    truncated: bool,
...
async fn start_provider_teach(
    ...
    provider
        .eval(PROVIDER_TEACH_SCRIPT.replace("__CHATCHAT_ROLE_JSON__", &role_json))
        .map_err(|error| error.to_string())?;
...
async fn cancel_provider_teach(
    ...
    provider
        .eval("(() => { window.__CHATCHAT_TEACH_CLEANUP__?.(); window.__CHATCHAT_TEACH_SELECTION__ = null; })()")
        .map_err(|error| error.to_string())
}

#[tauri::command]
async fn test_provider_speech(
    app_handle: tauri::AppHandle,
    webview_window: tauri::WebviewWindow,
    request: ProviderSpeechRequest,
) -> Result<ProviderSpeechResult, String> {
    ensure_main_caller(&webview_window)?;
    perform_provider_speech(
        &app_handle,
        &request,
        4_000,
...
#[tauri::command]
async fn run_provider_council_turn(
    app_handle: tauri::AppHandle,
    webview_window: tauri::WebviewWindow,
    request: ProviderSpeechRequest,
) -> Result<ProviderSpeechResult, String> {
...
async fn perform_provider_speech(
    app_handle: &tauri::AppHandle,
    request: &ProviderSpeechRequest,
    ...
    validate_speech_request(request, max_message_characters, operation_label)?;
    let provider = provider_window_for_host(
        app_handle,
        &request.profile_id,
        &request.expected_origin,
    ...
    run_browser_action(
        &provider,
        build_compose_script(&request.composer_selector, &request.message),
        "composer",
    )
    .await?;
    ...
    run_browser_action(
        &provider,
        build_send_script(&request.send_selector),
        "send",
    )
    .await?;
    ...
    loop {
        ...
        if started.elapsed() >= timeout {
            return Err(format!(
                "{operation_label} timed out after {}s. Taught response matches: {} (baseline {}). Last response was{} truncated.",
                timeout.as_secs(),
                latest_count,
                baseline.count,
...
fn validate_speech_request(
    ...
    let count = request.message.chars().count();
    if request.message.trim().is_empty() || count > max_message_characters {
        return Err(format!(
            "{operation_label} message must contain 1-{max_message_characters} characters."
        ));
    }
    for (name, selector) in [
        ("composer", &request.composer_selector),
        ("send", &request.send_selector),
        ("response", &request.response_selector),
    ] {
        if selector.trim().is_empty() || selector.len() > 512 || selector.contains('\0') {
            return Err(format!("Invalid {name} selector."));
        }
    }
    Ok(())
}
...
async fn run_browser_action(
    ...
    let result: BrowserActionResult = serde_json::from_value(value)
        .map_err(|error| format!("Invalid {label} action result: {error}"))?;
    if result.ok {
        Ok(())
    } else {
        Err(result
            .error
            .unwrap_or_else(|| format!("{label} action failed.")))
    }
}
...
async fn capture_response(
    window: &tauri::WebviewWindow,
    selector: &str,
) -> Result<BrowserResponseSnapshot, String> {
    let value = eval_json(window, build_capture_script(selector)).await?;
    let snapshot: BrowserResponseSnapshot = serde_json::from_value(value)
        .map_err(|error| format!("Invalid response capture result: {error}"))?;
    if snapshot.ok {
        Ok(snapshot)
    } else {
        Err(snapshot
            .error
            .unwrap_or_else(|| "Response capture failed.".into()))
    }
}
...
fn build_compose_script(selector: &str, message: &str) -> String {
    PROVIDER_SPEECH_COMPOSE_SCRIPT
        .replace(
            "__CHATCHAT_COMPOSER_SELECTOR_JSON__",
            &serde_json::to_string(selector).expect("selector JSON"),
        )
        .replace(
            "__CHATCHAT_MESSAGE_JSON__",
            &serde_json::to_string(message).expect("message JSON"),
        )
}

fn build_send_script(selector: &str) -> String {
    PROVIDER_SPEECH_SEND_SCRIPT.replace(
        "__CHATCHAT_SEND_SELECTOR_JSON__",
        &serde_json::to_string(selector).expect("selector JSON"),
    )
}

fn build_capture_script(selector: &str) -> String {
    PROVIDER_SPEECH_CAPTURE_SCRIPT.replace(
        "__CHATCHAT_RESPONSE_SELECTOR_JSON__",
        &serde_json::to_string(selector).expect("selector JSON"),
    )
}
...
fn provider_window_for_host(
    ...
    if !same_provider_host(&current, &expected) {
        return Err(format!(
            "Provider is currently on {}. Navigate back to {} before using the Browser Council Bridge.",
