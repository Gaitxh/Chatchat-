<div align="center">
  ...
  <p><strong>你只提交一次提案。接下来，让一桌 AI 自己把会开完。</strong><br />ChatChat 自动召集平等参与者：先独立思考，再分工研究、公开质疑、拿出证据、直接追问、明确回应；有人真的被说服时，改口因果也会当场出现。仍未解决的问题不会被结果页偷偷抹掉。</p>
...
replyToEventId / targetEventId / causedBy
   ...
   ↓
Live Persuasion
   只有协议已经证明的 revision / concede 才算真的说服
   ↓
Open Issues
   “哪些问题到现在仍没有得到明确回应？”
   ↓
...
> README 里的总览图不是未来概念图。它只画已经进入生产代码、并被真实 Chromium 产品门禁验证过的会议能力。

---

## 01 · 每个 AI 都像一个正在办案的人

大会进行时，你可以看到很多“活人感”，但这些 UI 都只读取公开结构化事实，不展示、更不会伪造模型的私有思维链。

<table>
<tr><td><strong>LIVE PARTICIPANTS</strong><br/>谁在独立分析、研究、公开回应、完成或失败。</td><td><strong>Live Research Desk</strong><br/>每个 AI 自动领取不同研究任务，例如主源核验、最强反例、实现约束；任务不同，权力相同。</td></tr>
...
<tr><td><strong>LIVE PERSUASION</strong><br/>只有 canonical influence graph 已经判定为 strong 的 revision.causedBy / concede 才显示“谁说服了谁”。</td><td><strong>RELATIONSHIP MAP</strong><br/>支持、质疑、证据、直接回复、明确改口会长成可点击的关系边。</td></tr>
<tr><td><strong>Meeting Secretariat</strong><br/>Live Agenda 解释为什么会议继续；Open Issues 列出仍未得到结构化回应的问题。</td><td><strong>CONSULTATION THEATER</strong><br/>会后重新播放公开事件、改口轨迹和精确影响来源，不重新调用 Provider。</td></tr>
</table>
...
⚔ ChatGPT → Claude
  “什么真正需要 extension-first UI，而不是 Web Room + 隐形 bridge？”
...
“被说服”也不是从一句“你说得对”里猜。**Live Persuasion** 直接复用 canonical influence graph：只有明确的 `revision.causedBy` 或 `concede` 形成 strong edge，才会出现 `influencer → changing AI`；普通 support、challenge、evidence 本身都只是互动，除非后续真的促成立场变化。

因此同一段 `Claude → ChatGPT → 回答 → Claude 改口` 可以同时出现在：

- Live Discussion Stream 的回答卡；
- Peer Exchange Queue 的 `已排队 → 目标 AI 接手 → 已回答` 生命周期；
- Live Persuasion 的强影响卡；
- Relationship Map 的 `reply` / influence 关系；
- 原始事件详情里的精确 provenance。

这才叫“互相说服”，而不是几个模型各写各的。

---

## 03 · 大会自己知道“为什么还要继续”

ChatChat 的**大会秘书处**不是一个隐藏主持人，也不是额外的 AI。它只是协商协议之上的确定性视图。

**Live Agenda** 的继续原因由 orchestrator 直接发布：
...
当原因是 `fresh_signal_follow_up` 时，Agenda 会带真正触发下一轮的 `triggerEventIds`，用户可以点回原始 Evidence / Challenge / Question，而不是看一段会后编出来的解释。
...
```text
领先立场：Web Room + Invisible Bridge
对齐度：75%
...
- 明确的 evidence / challenge → revision / concede 强影响链；
- 谁回答了谁，以及精确引用了哪条公共事件；
- 来源观察与证据状态；
...
README 和正式 Demo 也受 `scripts/check-readme-product-truth.mjs` 与 `tests/demo-output.test.mjs` 约束：核心叙事如果没有对应生产实现、Chromium proof，或终端 Demo 又退回层级化旧语义，产品检查就不能通过。
