...
/** Fixed, reviewable, stance-neutral investigative jobs. */
...
export function committeeById(id: HouseCommitteeId): HouseCommitteeDefinition {
...
/**
 * Deterministically spread Provider delegations across selected committees.
 * First minimize same-delegation concentration, then total committee size.
 */
export function assignHouseCommittees(
  participants: readonly CouncilParticipant[],
  committeeIds: readonly HouseCommitteeId[] = BUILT_IN_HOUSE_COMMITTEES.map((item) => item.id),
): CommitteePlan {
  const committees = [...new Set(committeeIds)].map(committeeById);
  if (!committees.length || !participants.length) {
    return { committees, assignments: [], unassigned: [...participants] };
  }

  const ordered = [...participants].sort(participantAssignmentOrder);
  const totals = new Map<HouseCommitteeId, number>(committees.map((committee) => [committee.id, 0]));
  ...
  for (const participant of ordered) {
    ...
    const perDelegation = delegationTotals.get(delegationKey) ?? new Map<HouseCommitteeId, number>();
  ...
  return { committees, assignments, unassigned: [] };
}
...
export function committeeComposition(plan: CommitteePlan) {
  return plan.committees.map((committee) => {
    const members = plan.assignments.filter((assignment) => assignment.committee.id === committee.id);
    return {
      committee,
      seats: members.length,
      delegations: new Set(
        members.map(({ participant }) => participant.delegationId ?? participant.provider),
      ).size,
...
/** Pick a compact number of lenses for smaller Houses. */
export function recommendedCommitteeIds(seatCount: number): HouseCommitteeId[] {
  if (seatCount <= 2) return ["evidence", "counterexample"];
  if (seatCount <= 4) return ["evidence", "engineering", "counterexample", "requirements"];
  if (seatCount <= 8) return ["evidence", "security", "engineering", "experience", "counterexample"];
  return BUILT_IN_HOUSE_COMMITTEES.map((item) => item.id);
}

function participantAssignmentOrder(a: CouncilParticipant, b: CouncilParticipant): number {
  ...
  return delegationA.localeCompare(delegationB) || a.id.localeCompare(b.id);
}
