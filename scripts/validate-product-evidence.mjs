...
requireAll("Chinese Side Panel", pages.sideZh, [
  "用户发起提案", "平等 AI 参与者", "没有议长", "协商结果", "协商剧场", "明确发生的立场修正", "明示立场战线", "会议最终席位图", "上下文记忆收据", "PUBLIC PAYLOAD INTEGRITY", "Claude", "Web + Extension", "Browser Extension",
]);
requireAll("English Side Panel", pages.sideEn, [
  "Propose once", "Equal AI participants", "CONSULTATION OUTCOME", "CONSULTATION THEATER", "Explicit revisions", "Who changed what", "EXPLICIT STANCE FRONTS", "FINAL POSITION FLOOR", "PROVIDER MEMORY COVERAGE", "PUBLIC PAYLOAD INTEGRITY", "Claude", "Web + Extension", "Browser Extension",
]);
...
requireAll("Chinese Full Room", pages.roomZh, ["协商记录", "history-entry", "INDEXEDDB · LOCAL", "协商剧场", "AI 关系战场", "明示立场战线", "会议最终席位图", "LOCAL · EXECUTION RECEIPT", "历史回放：只读取冻结的 execution receipt", "PUBLIC PAYLOAD INTEGRITY"]);
requireAll("English Full Room", pages.roomEn, ["CONSULTATION HISTORY", "history-entry", "INDEXEDDB · LOCAL", "CONSULTATION THEATER", "RELATIONSHIP MAP", "EXPLICIT STANCE FRONTS", "FINAL POSITION FLOOR", "LOCAL · EXECUTION RECEIPT", "Archive replay: reconstructed only from frozen execution receipts", "PUBLIC PAYLOAD INTEGRITY"]);
...
console.log("✓ bilingual Chromium DOM evidence preserves consultation, conflict, stance-front, final-position, execution, Provider-memory, serialized-payload and frozen-history contracts");
