(() => {
  ...
  const pack = {
    ...
    council: {
      mode: "live",
      sessionFingerprint: "showcaseB001",
  ...
  const localState = {};
  const sessionState = { [proofKey]: pack };
  const showcaseChrome = {
    ...
    tabs: {
      query: async () => [],
      get: async () => undefined,
      create: async ({ url, active }) => ({ id: 901, url, active: Boolean(active), status: "complete" }),
      update: async (id, patch) => ({ id, ...patch, status: "complete" }),
      sendMessage: async () => ({ ok: false, error: "Gate B showcase has no live Provider tabs." }),
  ...
  window.addEventListener("load", () => {
    ...
    badge.textContent = "DETERMINISTIC GATE B SHOWCASE · METADATA ONLY · NO REAL ACCOUNT";
    badge.style.cssText =
      "position:fixed;z-index:999999;right:8px;bottom:8px;padding:6px 8px;border-radius:999px;background:#205d45;color:#fff;font:700 8px system-ui;letter-spacing:.05em;opacity:.9;pointer-events:none";
