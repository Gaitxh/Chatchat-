(() => {
  ...
  const seats = [
    ...
    seat(201, "alibaba-tongyi", "Qwen / Tongyi", "https://tongyi.aliyun.com", "qwen", "Qwen Delegation"),
    seat(202, "alibaba-tongyi", "Qwen / Tongyi", "https://tongyi.aliyun.com", "qwen", "Qwen Delegation"),
    seat(203, "alibaba-tongyi", "Qwen / Tongyi", "https://tongyi.aliyun.com", "qwen", "Qwen Delegation"),
    seat(204, "alibaba-tongyi", "Qwen / Tongyi", "https://tongyi.aliyun.com", "qwen", "Qwen Delegation"),
    seat(205, "alibaba-tongyi", "Qwen / Tongyi", "https://tongyi.aliyun.com", "qwen", "Qwen Delegation"),
  ];
  ...
  const storage = {
    [recipesKey]: {
      ...
      "https://tongyi.aliyun.com": recipe("https://tongyi.aliyun.com"),
    },
  };
  ...
  // Normal Chrome pages expose a partial window.chrome object even though they
  // do not have extension APIs. Mutate that object instead of assuming we can
  // replace the browser-owned global wholesale.
  ...
  function councilResponse(prompt, tabId) {
    ...
    if (phase === "sealed") {
      ...
      const cause =
        publicEvents.find(
          (event) =>
            event.actorId.includes("alibaba-tongyi") &&
            event.kind === "argument" &&
            event.stance === "Tauri",
        ) ??
        publicEvents.find(
  ...
  async function autoRun() {
    try {
      const firstTest = await waitEnabled(
        ".teach-actions button:last-child",
        0,
      );
      firstTest.click();
      await sleep(650);
      const secondTest = await waitEnabled(
        ".teach-actions button:last-child",
        1,
      );
      secondTest.click();
