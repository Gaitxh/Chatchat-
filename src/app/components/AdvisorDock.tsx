  ...
  return (
    <section className="advisor-dock">
      <header className="advisor-dock__header">
        <div><span className="eyebrow">ADVISOR ROSTER · 智囊名册</span><h2>Teach ChatChat a new advisor</h2><p>登录 → 御前试音 → 点选输入框 / 发送按钮 / 回答区域。配方只存在本地，不需要用户手写 CSS selector。</p></div>
          ...
          return (
            <article className="advisor-profile" key={profile.profileId}>
              ...
              <div className="advisor-profile__body">
                ...
                {loginOpen ? <RecipeCard profile={profile} recipe={recipe} teaching={teaching} disabled={props.disabled || adapterMissing} onTeach={props.onTeach} onCancel={props.onCancelTeach} /> : null}
              </div>
              <div className="advisor-profile__actions"><button type="button" className="login-advisor" disabled={props.disabled || adapterMissing || !props.canOpenLogin} onClick={() => void props.onLogin(profile)}>{loginOpen ? "FOCUS LOGIN" : "LOGIN"}</button><button type="button" className="probe-advisor" disabled={props.disabled || !loginOpen || probing || adapterMissing} onClick={() => void props.onProbe(profile)}>{probing ? "试音中…" : "御前试音"}</button><button type="button" className="remove-advisor" onClick={() => void props.onRemove(profile.profileId)} disabled={props.disabled}>移除</button></div>
            </article>
          );
        })}
      </div>
      <div className="provider-catalog-strip">{BUILT_IN_PROVIDER_MANIFESTS.map((manifest)=><span key={manifest.id}>{manifest.monogram} · {manifest.displayName}</span>)}<span>Teach Mode · 3 clicks → local recipe</span></div>
    ...
    <div className="recipe-card">
      ...
      <div className="recipe-steps">
        {(["composer","send","response"] as const).map((role) => {
          ...
          return <div className={`recipe-step ${selector ? "recipe-step--done" : ""} ${active ? "recipe-step--active" : ""}`} key={role}><div><b>{roleIcon(role)} {roleLabel(role)}</b><code>{selector ?? "not taught"}</code></div><button type="button" disabled={disabled || Boolean(teaching && !active)} onClick={() => active ? void onCancel(profile) : void onTeach(profile, role)}>{active ? "取消点选" : selector ? "重教" : "教我"}</button></div>;
        })}
      </div>
      {teaching ? <div className="teach-banner">✨ Provider 窗口已经进入点选模式。移动鼠标会出现高亮，点击目标元素即可记录；页面内容不会被读取。</div> : adapterRecipeComplete(recipe) ? <div className="recipe-complete">✓ 三个定位已齐。下一阶段 Browser Adapter 可以开始练习发言。</div> : null}
    </div>
  );
}
