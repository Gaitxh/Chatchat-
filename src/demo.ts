...
import { deriveLivePersuasionMoments } from "./theater/live-persuasion.js";
...
const announcedPersuasion = new Set<string>();
...
for (const participant of participants) {
  ...
  if (!lane) continue;
  console.log(`- ${participant.name}: ${researchLaneDefinition(lane).en.label}`);
}
...
const { report, blackboard } = await orchestrator.run(proposal, {
  ...
  onPhase(update) {
    ...
    printOpenIssues("OPEN ISSUES BEFORE THIS PHASE", deriveOpenMeetingIssues(participants, liveEvents));
  },
  ...
  onEvent(event) {
    ...
    for (const moment of deriveLivePersuasionMoments(participants, liveEvents)) {
      if (announcedPersuasion.has(moment.id)) continue;
      announcedPersuasion.add(moment.id);
      console.log("\n✨ LIVE PERSUASION");
      console.log(`${moment.influencerName} → ${moment.changingActorName} · ${moment.kind} · R${moment.round}`);
      if (moment.fromStance || moment.toStance) console.log(`stance: ${moment.fromStance ?? "?"} → ${moment.toStance ?? "?"}`);
      console.log(`causeEventId: ${moment.causeEventId} · ${moment.causeKind}`);
      console.log(`actionEventId: ${moment.actionEventId}`);
      console.log(`cause: ${moment.causeExcerpt}`);
    }
  },
});
...
printOpenIssues("OPEN ISSUES AT CLOSE", deriveOpenMeetingIssues(participants, blackboard.events), true);
...
function printOpenIssues(
  title: string,
  issues: ReturnType<typeof deriveOpenMeetingIssues>,
  showEmpty = false,
): void {
...
function agendaReason(reason: CouncilPhaseReason | undefined): string {
  switch (reason) {
    case "sealed_start": return "independent first positions are being formed without peer answers";
    case "initial_debate": return "sealed positions are now visible in one shared public snapshot";
    case "fresh_signal_follow_up": return "the prior parallel batch introduced fresh questions/evidence/revisions that peers deserve a chance to answer";
    case "minimum_debate_rounds": return "the selected consultation mode requires more public debate";
    case "alignment_not_reached": return "descriptive stance alignment remains below the configured threshold";
    case "finalizing_stable_alignment": return "alignment is stable and the prior batch introduced no fresh peer-response signal";
    case "finalizing_round_budget": return "the hard round boundary was reached; remaining disagreement and Open Issues are preserved";
    default: return "protocol phase transition";
  }
}
