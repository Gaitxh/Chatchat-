...
export function ProviderMemoryPayloadIntegrity({ model, locale, archive = false }: ProviderMemoryPayloadIntegrityProps) {
  ...
  return (
    ...
    >
      ...
      <footer>{zh
        ? "eq64 只对真正进入 RUN_SPEECH 的 CONSULTATION_EVENTS_JSON 原始序列化 UTF-8 文本做同场相等性对账；它不是密码学签名、真实性证明、证据质量分或答案正确率。Repair 还必须保留相同 snapshot / pinned / pin-source / latest provenance。缺少 Prompt payload 票据的真实 Provider turn 仍留在分母里，绝不会靠消失变成 verified。"
        : "eq64 compares the raw serialized UTF-8 CONSULTATION_EVENTS_JSON text that actually entered RUN_SPEECH inside one meeting only. It is not a cryptographic signature, authenticity proof, evidence-quality score, or answer-correctness signal. Repair must also preserve the same snapshot / pinned / pin-source / latest provenance. A real Provider turn missing Prompt-payload evidence remains in the denominator; it never disappears to manufacture verification."}</footer>
    </section>
  );
}
...
function repairLabel(round: ProviderPublicPayloadRound, zh: boolean): string {
  ...
  if (round.repairUnverifiedSeatCount > 0) return `${round.repairUnverifiedSeatCount} ${zh ? "个 repair 票据不完整" : "repair context unverified"}`;
  if (round.repairDriftSeatCount > 0) {
    const parts: string[] = [];
    if (round.repairPayloadDriftSeatCount) parts.push(`${round.repairPayloadDriftSeatCount} ${zh ? "序列化 payload 漂移" : "serialized payload drift"}`);
    if (round.repairSelectionDriftSeatCount) parts.push(`${round.repairSelectionDriftSeatCount} ${zh ? "selection provenance 漂移" : "selection provenance drift"}`);
