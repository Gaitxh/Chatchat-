...
export function ProviderMemoryFairness({ model, locale, archive = false }: ProviderMemoryFairnessProps) {
  ...
  return (
    <section
      ...
      data-memory-fairness-metadata-mismatch-turns={model.promptMetadataMismatchTurns}
    ...
    >
      ...
      <div className="provider-memory-fairness__metrics">
        ...
        <span><b>{model.promptMetadataMismatchTurns}</b><small>{zh ? "metadata↔payload 漂移" : "metadata↔payload drifts"}</small></span>
      ...
      <footer>{archive
        ...
        : (zh ? "这里审计的是程序：谁被公共上下文代表、平等席位是否拿到相同 payload、Prompt 自报 IDs 是否与实际 payload 相同、repair 是否保持同一副牌。它不判断任何 AI 的答案是否正确。" : "This audits procedure: actor representation, equal-seat public payload equality, declared snapshot ids versus actual payload ids, and repair-deck parity. It does not judge whether any AI answer is correct.")}</footer>
    </section>
  );
}
...
function FairnessRound({ round, zh }: { round: ProviderMemoryFairnessRound; zh: boolean }) {
  ...
  const ok = round.latestRoundRepresentationComplete
    && round.publicPayloadConsistent
    && !round.promptMetadataMismatchSeats
    && !round.selectorActorMismatchSeats
    && !round.repairContextMismatchSeats;
  return (
    <article
      className={ok ? "is-ok" : "has-issue"}
      ...
      data-memory-fairness-metadata-mismatch-seats={round.promptMetadataMismatchSeats}
    ...
    >
      ...
      <ul>
        ...
        <li className={round.promptMetadataMismatchSeats ? "bad" : "ok"}>{round.promptMetadataMismatchSeats ? "!" : "✓"} {zh ? "metadata = actual IDs" : "metadata = actual IDs"}</li>
...
function stateLabel(state: ProviderMemoryFairnessState): string {
  ...
  if (state === "prompt_metadata_drift") return "PROMPT METADATA DRIFT";
...
function title(state: ProviderMemoryFairnessState, zh: boolean): string {
  ...
  if (state === "prompt_metadata_drift") return zh ? "Prompt 自报的 snapshot IDs 与实际公共 payload 不一致" : "The Prompt's declared snapshot ids disagree with its actual public payload";
...
function body(state: ProviderMemoryFairnessState, zh: boolean): string {
  if (state === "verified") return zh ? "验证内容包括：最新轮席位代表、实际 CONSULTATION_EVENTS_JSON fingerprint、metadata↔actual IDs、selector actor coverage，以及 repair 前后公共 deck 一致性。" : "Verified dimensions include latest-round actor representation, actual CONSULTATION_EVENTS_JSON fingerprints, metadata↔actual ids, selector actor coverage, and first/repair public-deck parity.";
  ...
  if (state === "prompt_metadata_drift") return zh ? "PUBLIC_SNAPSHOT_EVENT_IDS_JSON 不能给自己作证。ChatChat 会从 CONSULTATION_EVENTS_JSON 独立提取 actual IDs；两者不同就直接披露。" : "PUBLIC_SNAPSHOT_EVENT_IDS_JSON cannot certify itself. ChatChat independently extracts actual ids from CONSULTATION_EVENTS_JSON and reports any mismatch.";
  if (state === "repair_context_drift") return zh ? "Repair 只应修输出格式。actual/declaration snapshot、pin、actor coverage 或 payload fingerprint 变化都会被视为程序漂移。" : "Repair is format-only. Any change to actual/declared snapshot, pins, actor coverage, or payload fingerprint is procedural drift.";
