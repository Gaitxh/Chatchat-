...
/**
 * Find canonical unresolved obligations that existed at turn start but were not
 * present in that Provider's bounded public Prompt snapshot.
 *
 * A gap is a memory-coverage fact only. It is not a semantic importance score,
 * proof of unfairness, or claim that a different answer would have resulted.
 */
export function deriveProviderMemoryGaps(
  ...
  const gaps = coverage.turns.flatMap((turn) => deriveTurnGaps(turn, eventById, participantById));
  ...
  return {
    ...
    legacyGapCount: gaps.filter((gap) => gap.selectionEvidence === "legacy_selector_audit").length,
