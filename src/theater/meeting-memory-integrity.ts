 ...
 * answer confidence, answer correctness or a synthetic composite trust score.
 */
export function deriveMeetingMemoryIntegrity(
  ...
  const protocolState: MeetingMemoryProtocolState = coverage.turns.length > 0 && coverage.legacySelectorTurnCount === coverage.turns.length
    ? "legacy_unverified"
  ...
  return {
    ...
    legacySelectorTurns: coverage.legacySelectorTurnCount,
...
function evidenceStrength(coverage: ProviderMemoryCoverageModel): MeetingMemoryEvidenceStrength {
  ...
  if (coverage.legacySelectorTurnCount === coverage.turns.length) return "legacy_selector_audit";
  if (coverage.actualPromptTurnCount > 0) return "mixed";
