...
export function formatEvent(event: CouncilEvent, actorName: string): string {
  ...
  switch (event.kind) {
    case "argument":
      return join([headline, `   event: ${event.id}`, `   stance: ${event.stance} (${Math.round(event.confidence * 100)}%)`, `   ${event.content}`, ...provenance]);
    case "revision":
      return join([
        headline,
        `   event: ${event.id}`,
    ...
    case "defense":
    case "concede":
      return join([headline, `   event: ${event.id}`, `   targetEventId: ${event.targetEventId}`, `   ${event.content}`]);
    case "question":
      return join([headline, `   event: ${event.id}`, ...(event.targetActorId ? [`   targetActorId: ${event.targetActorId}`] : []), `   ${event.content}`, ...provenance]);
    case "uncertain":
      return join([headline, `   event: ${event.id}`, `   confidence: ${Math.round(event.confidence * 100)}%`, `   ${event.content}`, ...provenance]);
  }
}
...
export function formatReport(report: CouncilReport): string {
  ...
  const positions = report.positions.length
    ? report.positions.map((position) => `- ${position.participant.name}: ${position.stance} (${Math.round(position.confidence * 100)}%)`).join("\n")
    : "None";
  const minority = report.disagreements.length
    ? report.disagreements.map((position) => `- ${position.participant.name}: ${position.stance} — ${position.content}`).join("\n")
    : "None";
...
function eventProvenance(event: CouncilEvent): string[] {
  if ((event.kind === "argument" || event.kind === "evidence" || event.kind === "question" || event.kind === "uncertain") && event.replyToEventId) {
    return [`   replyToEventId: ${event.replyToEventId}`];
  }
  return [];
}
