...
export interface FinalPromptReceipt {
  ...
  actorId: string;
  round: number;
  /** Exact initial Final protocol input sent through RUN_SPEECH. Local-only; never Provider output or hidden reasoning. */
  promptText: string;
  promptChars: number;
...
export function createFinalPromptReceipt(promptText: string, observedAt = new Date().toISOString()): FinalPromptReceipt {
  const parsed = parseIdentity(promptText);
...
export function validateFinalPromptReceipt(receipt: FinalPromptReceipt): FinalPromptReceiptValidation {
  const parsed = parseIdentity(receipt.promptText);
  const issues: string[] = [];
  ...
  return { valid: issues.length === 0, issues, currentFingerprint, parsedSnapshotEventIds: parsed.snapshotEventIds };
}
...
function parseIdentity(promptText: string): {
  sessionId?: string;
  phase?: string;
  round?: number;
  actorId?: string;
  ...
  return {
    ...
    ...(actorId ? { actorId } : {}),
    snapshotEventIds: parseSnapshotIds(snapshotText),
...
function parseSnapshotIds(value: string | undefined): string[] {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) throw new Error("PUBLIC_SNAPSHOT_EVENT_IDS_JSON must be an array of event IDs.");
  return [...parsed];
}
