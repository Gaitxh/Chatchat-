...
export function deriveOpenMeetingIssues(
  ...
  for (const event of events) {
    if (event.kind === "question") {
      ...
      if (!responses.length) {
        result.push(issue("open_question", event, participantNames, []));
      }
      continue;
    }
    ...
    if (event.kind === "challenge") {
      ...
      if (!responses.length) {
        result.push(issue(
          ...
          target ? `${actor(participantNames, event.actorId)} challenged ${actor(participantNames, target.actorId)}: ${event.content}` : event.content,
        ));
      }
      continue;
    }
    ...
    if (event.kind === "uncertain") {
      ...
      if (!resolved) {
        result.push(issue("explicit_uncertainty", event, participantNames, []));
      }
    }
  }
