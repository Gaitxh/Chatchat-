...
import {
  ...
  type FinalPromptReceipt,
} from "./final-prompt-receipt.js";
import type {
  FinalCoverageGap,
  FinalCoveragePlan,
...
/**
 * Build safe recovery packets. A retry is READY only when ChatChat has the exact
 * initial Final prompt that was actually sent, the prompt receipt validates,
 * its embedded snapshot IDs match the frozen attendance receipt, and every
 * referenced event still exists in the immutable meeting archive.
 */
export function deriveFinalRecoveryPackets(
  ...
  const promptByActor = new Map(finalPromptReceipts
    .filter((receipt) => receipt.sessionId === report.sessionId)
    .map((receipt) => [receipt.actorId, receipt] as const));

  const packets = coverage.gaps.map((gap) => {
    const participant = participantById.get(gap.actorId);
    if (gap.recommendedOperation === "request_final_shift_explanation") {
      return nonRetryPacket(report, gap, participant, "clarification_only");
    }
    if (gap.recommendedOperation === "verify_final_execution_provenance") {
      return nonRetryPacket(report, gap, participant, "verification_only");
    }
...
export function finalRecoveryPacketText(packet: FinalRecoveryPacket): string {
  ...
  if (packet.state === "ready_exact_prompt") {
    lines.push(
      ...
      "The stored exact Final Prompt is protocol input only; this summary intentionally does not print its full text.",
    );
...
function nonRetryPacket(
  report: CouncilReport,
  gap: FinalCoverageGap,
  participant: CouncilParticipant | undefined,
  state: "verification_only" | "clarification_only",
): FinalRecoveryPacket {
  return {
    ...packetBase(report, gap, participant),
    state,
...
function blockedPacket(
  ...
  gap: FinalCoverageGap,
  participant: CouncilParticipant | undefined,
  state: Exclude<FinalRecoveryPacketState, "ready_exact_prompt" | "verification_only" | "clarification_only" | "blocked_missing_snapshot_events">,
  ...
  return {
    ...
    ...(receipt ? {
      exactFinalPrompt: receipt.promptText,
      promptFingerprint: receipt.promptFingerprint,
      promptChars: receipt.promptChars,
      snapshotEventIds: [...receipt.snapshotEventIds],
    } : { snapshotEventIds: [] }),
    snapshotEvents: [],
    missingSnapshotEventIds: [],
